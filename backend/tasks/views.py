...
def create_task(request):
    ...
    try:
        ...
        if date_formatted < datetime.now().strftime("%Y-%m-%d %H:%M:%S"):
            return JsonResponse(
                {"error": "La fecha debe ser posterior a la actual"}, status=400
            )
        ...
        for tag_id in tags:
            try:
                ...
                return JsonResponse(
                    {"error": f"La etiqueta con ID {tag_id} no existe"}, status=400
                )
...
def get_tasks(request):
    try:
        if request.method != "GET":
            return JsonResponse(
                {"error": "El unico metodo permitido es GET"}, status=405
            )
        ...
        try:
            ...
            return JsonResponse(
                {"error": f"Error al obtener las tareas: {str(e)}"}, status=500
            )
...
def update_task(request):
    ...
    try:
        ...
        try:
            ...
            return JsonResponse(
                {"message": "Tarea actualizada correctamente"}, status=200
            )

        except Exception as e:
            return JsonResponse(
                {"error": f"Error al actualizar la tarea: {str(e)}"}, status=500
            )
    except Exception as e:
        return JsonResponse(
            {"error": f"Error al actualizar la tarea: {str(e)}"}, status=500
...
def get_recommendations(request):
    try:
        ...
        try:
            ...
            return JsonResponse(
                {"error": f"Error al realizar la inferencia: {str(e)}"}, status=500
            )
        ...
        return JsonResponse(
            {"error": f"Error al obtener las recomendaciones: {str(e)}"}, status=500
        )
...
def delete_task(request, id):
    if request.method != "DELETE":
        return JsonResponse(
            {"error": "El único metodo permitido es DELETE"}, status=405
        )
    ...
    try:
        ...
        return JsonResponse(
            {"error": f"Error al eliminar la tarea: {str(e)}"}, status=500
        )
...
def send_reminder_email(task):
    subject = f"Recordatorio: Tu ea {task.title} vence pronto"
    message = (
        f"Hola {task.user.name}, tu tarea {task.title} termina el dia {task.end_date}"
    )
...
def delete_tag(request, id):
    ...
    try:
        ...
        tag.delete()
        return JsonResponse({"message": "Categorua eliminada correctamente"}, status=200)
...
def update_tag(request):
    ...
    try:
        ...
        try:
            ...
            return JsonResponse({"message":"Categoria actualizada correctamente"}, status=200)

        except Exception as e:
            return JsonResponse({"error":"Error al actualizar la categoria"}, status=500)
